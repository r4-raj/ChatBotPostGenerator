...
  // If already authenticated, decide where to go based on profile existence
  useEffect(() => {
    const token = sessionStorage.getItem('authToken'); // Changed to sessionStorage
    if (!token) return;
...
        }
      })
      .catch(() => router.replace('/businesses/create'));
  }, [router]);
...
      
      if (data.token) {
        sessionStorage.setItem('authToken', data.token); // Changed to sessionStorage
        console.log('Login successful!');
        // Redirect to a dashboard or home page
        // After login, set flash and check if profile exists
        sessionStorage.setItem('flashMessage', 'Signed in successfully');
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
        fetch(`${apiUrl}/profile/me`, { headers: { Authorization: `Bearer ${data.token}` }})
          .then((res) => {
            if (res.ok) router.push('/content/generate');
            else router.push('/businesses/create');
          })
          .catch(() => router.push('/businesses/create'));
      }
      
    } catch (err) {
...
         // After Google login, set flash and check if profile exists
